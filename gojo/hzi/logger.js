...
module.exports = {
    Normal: function(/** @type {string} */ Str, /** @type {() => any} */ Data, /** @type {() => void} */ Callback) {
        if (isHexcolor(global.Fca.Require.FastConfig.MainColor) != true) {
            ...
            // Chỉ hiển thị thông báo nếu không phải là FCA hoặc nếu Str không rỗng
            if (global.Fca.Require.FastConfig.MainName !== "[ FCA ]" || Str.trim() !== "") {
                // Tạo gradient cho prefix giống như [ HARU ]
                const prefix = createGradientText(`${global.Fca.Require.FastConfig.MainName || '[ FCA-HZI ]'} > `, global.Fca.Require.FastConfig.MainColor, '#0000FF');
